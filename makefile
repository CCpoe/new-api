...
.PHONY: all build-web build-web-classic build-web-image build-all-web start-api dev dev-api dev-api-rebuild dev-web dev-web-classic dev-web-image reset-setup
...
build-web-image:
	@echo "Building image generation workbench..."
	@cd ./web && bun install --frozen-lockfile
	@cd $(WEB_IMAGE_DIR) && bun run build

build-all-web: build-web build-web-classic build-web-image
...
dev-web-image:
	@echo "Starting image generation workbench dev server..."
	@echo "Image workbench: http://localhost:$(DEV_WEB_IMAGE_PORT)"
	@cd ./web && bun install --filter ./image
	@cd $(WEB_IMAGE_DIR) && bun run dev -- --host 0.0.0.0 --port $(DEV_WEB_IMAGE_PORT)

...
async function readJsonServerSentEvents(
  ...
  if (!hasDataLine)
    throw new Error(
      appendStreamingFormatHint("未从流式响应中解析到有效的 data 事件"),
    );
}
...
function getResponsesImageResultBase64(
...
async function callImagesApiConcurrent(
  ...
  const successfulResults = results
    .filter(
      (r): r is PromiseFulfilledResult<CallApiResult> =>
        r.status === "fulfilled",
    )
    .map((r) => r.value);
  const failedRequests = results.flatMap((r, requestIndex) =>
    r.status === "rejected"
      ? [{ requestIndex, error: getErrorMessage(r.reason) }]
      : [],
  );
  ...
  const actualParamsList = successfulResults.flatMap((r) =>
    r.actualParamsList?.length
      ? r.actualParamsList
      : r.images.map(() => r.actualParams),
  );
...
async function pollCustomTaskResult(
  ...
  while (true) {
    ...
    if (state === "success") {
      try {
        return await extractCustomImages(
          taskPayload,
          poll.result,
          mime,
          signal,

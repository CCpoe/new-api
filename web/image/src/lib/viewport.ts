...
function isInsideLightbox(target: EventTarget | null) {
  return (
    target instanceof Element && Boolean(target.closest("[data-lightbox-root]"))
  );
}
...
export function installMobileViewportGuards() {
  ...
  const preventMultiTouchPageZoom = (event: TouchEvent) => {
    if (event.touches.length > 1 && !isInsideLightbox(event.target))
  ...
  document.addEventListener("touchmove", preventMultiTouchPageZoom, {
    passive: false,
  });
}

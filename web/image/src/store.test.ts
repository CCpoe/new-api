...
vi.mock("./lib/falAiImageApi", () => ({
  getFalErrorMessage: vi.fn((err: unknown) =>
...
vi.mock("./lib/transparentImage", () => ({
  ...
  removeKeyedBackgroundFromDataUrl: vi.fn(
    async (dataUrl: string) => `transparent:${dataUrl}`,
  ),
}));
...
describe("data operation locking", () => {
  it("detects running and recoverable work before import or export", () => {
    expect(hasActiveDataOperations([task({ status: "running" })], [])).toBe(
      true,
    );
    expect(hasActiveDataOperations([task({ falRecoverable: true })], [])).toBe(
      true,
    );
    expect(
...
describe("mask draft lifecycle in store actions", () => {
  ...
  it("stores decoded image size as actual size when the API omits size", async () => {
    ...
    expect(task.actualParams).toMatchObject({
      size: "1254x1254",
      output_format: "png",
      n: 1,
    });
    expect(task.actualParamsByImage?.[task.outputImages[0]]).toMatchObject({
      size: "1254x1254",
      output_format: "png",
    });
    await clearTasks();
  ...
  it("keeps API-returned actual size over decoded image size", async () => {
    ...
    expect(task.actualParamsByImage?.[task.outputImages[0]].size).toBe(
      "1024x1024",
    );
    await clearTasks();
    await clearImages();
  ...
  it("stores transparent background output after local post-processing", async () => {
    ...
    expect(outputImage?.dataUrl).toBe(
  ...
  it("falls back to the original output when transparent post-processing fails", async () => {
    ...
    vi.mocked(callImageApi).mockClear();
    vi.mocked(removeKeyedBackgroundFromDataUrl).mockClear();
    vi.mocked(removeKeyedBackgroundFromDataUrl).mockRejectedValueOnce(
  ...
  it("supports transparent background post-processing for fal gallery tasks", async () => {
    ...
    expect(removeKeyedBackgroundFromDataUrl).toHaveBeenCalledWith(
      "data:image/png;base64,fal-generated",
    );
    const [task] = useStore.getState().tasks;
...
describe("agent conversation persistence", () => {
  ...
  it("loads agent conversations from IndexedDB and migrates legacy localStorage conversations", async () => {
    ...
    expect(stored.map((conversation) => conversation.id)).toEqual([
      "stored-conversation",
      "legacy-conversation",
    ]);
  });
...
describe("fal task recovery", () => {
  ...
  it("applies transparent post-processing when a fal task recovers", async () => {
    ...
    expect(removeKeyedBackgroundFromDataUrl).toHaveBeenCalledWith(
      "data:image/png;base64,fal-recovered",
    );
    const recovered = useStore
      .getState()
      .tasks.find((item) => item.id === falTask.id);
    ...
    const originalImage = await getImage(
      recovered!.transparentOriginalImages![0],
    );
    expect(outputImage?.dataUrl).toBe(
      "transparent:data:image/png;base64,fal-recovered",
    );
  ...
  it("continues an Agent round after all fal image tasks recover", async () => {
    ...
    const imageProfile = createDefaultFalProfile({
      id: "fal-profile",
      apiKey: "fal-key",
    });
    ...
    for (
      let i = 0;
      i < 20 &&
      useStore.getState().agentConversations[0]?.rounds[0]?.status !== "done";
      i += 1
    ) {
      await new Promise((resolve) => setTimeout(resolve, 0));
    }

    const recoveredTask = useStore
      .getState()
      .tasks.find((item) => item.id === agentTask.id);
    ...
    const agentInputJson = JSON.stringify(
      vi.mocked(callAgentResponsesApi).mock.calls[0][0].input,
    );
    ...
    expect(round.responseOutput).toEqual(
  ...
  it("records recovered Agent tool failures without continuing the Agent round", async () => {
    ...
    vi.mocked(getFalQueuedImageResult).mockRejectedValueOnce(
    ...
    for (
      let i = 0;
      i < 20 &&
      useStore.getState().tasks.find((item) => item.id === agentTask.id)
        ?.falRecoverable !== false;
      i += 1
    ) {
      await new Promise((resolve) => setTimeout(resolve, 0));
    ...
    const toolOutput = round.responseOutput?.find(
  ...
  it("does not call Agent again when recovered tasks already reached the tool limit", async () => {
    ...
    expect(
      useStore
        .getState()
        .agentConversations[0].messages.find(
          (message) => message.id === "assistant-a",
        )?.content,
    ).toContain("已达到最大工具调用次数（1）");
  });
  ...
  it("does not continue a stopped Agent round when a recoverable fal task later completes", async () => {
    ...
    expect(
      useStore.getState().tasks.find((item) => item.id === agentTask.id),
    ).toMatchObject({ status: "done", falRecoverable: false });
  ...
  it("does not overwrite a stopped Agent task when an in-flight fal recovery completes", async () => {
    ...
    let resolveRecovery: (
      value: Awaited<ReturnType<typeof getFalQueuedImageResult>>,
    ) => void = () => {};
    vi.mocked(getFalQueuedImageResult).mockImplementationOnce(
      () =>
        new Promise((resolve) => {
    ...
    for (
      let i = 0;
      i < 20 && vi.mocked(getFalQueuedImageResult).mock.calls.length === 0;
      i += 1
    ) {
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
    useStore.setState((state) => ({
      agentConversations: state.agentConversations.map((item) =>
        item.id === "conversation-a"
          ? {
              ...item,
              rounds: item.rounds.map((round) =>
                round.id === "round-a"
                  ? { ...round, status: "running", error: null }
                  : round,
              ),
            }
          : item,
...
describe("agent round deletion", () => {
  ...
  it("collects agent round and conversation tasks even when some failed tasks are not in outputTaskIds", () => {
    ...
    const tasks = [
      ...
      task({
        id: "task-unrelated",
        agentConversationId: "other",
        agentRoundId: "other-round",
        status: "error",
        error: "失败",
      }),
    ];

    expect(getAgentRoundTaskIds(conversation.rounds[0], tasks)).toEqual([
      "task-success",
      "task-failed",
    ]);
    expect(getAgentConversationTaskIds(conversation, tasks)).toEqual([
      "task-success",
      "task-failed",
    ]);
  });
...
describe("data import", () => {
  ...
  it("restores favorite collections and default collection when importing task data", async () => {
    ...
    expect(state.favoriteCollections).toEqual(
      expect.arrayContaining(importedCollections),
    );
    expect(state.defaultFavoriteCollectionId).toBe(importedCollections[1].id);
    expect(
      state.tasks.find((item) => item.id === importedTask.id),
    ).toMatchObject({
      favoriteCollectionIds: [importedCollections[1].id],
      isFavorite: true,
    });
    expect(
      (await getAllTasks()).find((item) => item.id === importedTask.id),
    ).toMatchObject({
      favoriteCollectionIds: [importedCollections[1].id],
      isFavorite: true,
    });
  ...
  it("merges imported agent conversations without replacing local conversations", async () => {
    ...
    expect(
      state.agentConversations.map((conversation) => conversation.id),
    ).toEqual(["local-conversation", "imported-conversation"]);
  ...
  it("stores imported legacy agent conversations in IndexedDB without localStorage or image payloads", async () => {
      ...
      rounds: [
        {
          ...
          responseOutput: [
            ...
            },
            {
              type: "image_generation_call",
              id: "image-call-a",
              result: { base64: "imported-legacy-base64" },
            },
          ],
          status: "done",
          error: null,
    ...
    expect(imported).toBe(true);
    expect(indexedConversations.map((conversation) => conversation.id)).toEqual(
      ["legacy-imported-conversation"],
    );
    expect(serializedIndexedConversations).toContain("image_generation_call");
    expect(serializedIndexedConversations).not.toContain(
      "imported-legacy-base64",
    );
    expect("agentConversations" in persisted).toBe(false);
    expect(serializedPersisted).not.toContain("image_generation_call");
  ...
  it("imports a complete multipart backup selected in any order", async () => {
    ...
    const importedTask = task({
      id: "multipart-task",
      outputImages: ["multipart-image-a", "multipart-image-b"],
    });
    ...
    const imported = await importData([part2, part1], {
      importConfig: false,
      importTasks: true,
    });
    ...
    expect(imported).toBe(true);
    expect(
      (await getAllTasks()).some((item) => item.id === importedTask.id),
    ).toBe(true);
    expect(
      (await getAllTasks()).some((item) => item.id === "multipart-task-2"),
    ).toBe(true);
    expect(await getImage("multipart-image-a")).toMatchObject({
      dataUrl: "data:image/png;base64,AQI=",
    });
    expect(await getImage("multipart-image-b")).toMatchObject({
      dataUrl: "data:image/png;base64,AwQ=",
    });
  });
  ...
  it("rejects an incomplete multipart backup before importing data", async () => {
    ...
    const imported = await importData([part1], {
      importConfig: false,
      importTasks: true,
    });
    ...
    expect(
      (await getAllTasks()).some((item) => item.id === "incomplete-task"),
    ).toBe(false);
  });
  ...
  it("validates image entries in every part before writing earlier parts", async () => {
    ...
    const imported = await importData([part1, part2], {
      importConfig: false,
      importTasks: true,
    });
...
describe("agent context for removed outputs", () => {
  ...
  it("clears only failed gallery tasks", async () => {
    const failedA = task({
      ...
      status: "error",
      error: "生成失败",
      outputImages: ["failed-image-a"],
    });
    const failedB = task({
      id: "failed-b",
      status: "error",
      error: "生成失败",
      outputImages: ["failed-image-b"],
    });
    const done = task({
      id: "done-task",
      status: "done",
      outputImages: ["done-image"],
    });
    const running = task({
      id: "running-task",
      status: "running",
  ...
  it("keeps failed tasks created after the cleanup snapshot", async () => {
    const failedAtConfirmOpen = task({
      id: "failed-at-confirm-open",
      status: "error",
      error: "生成失败",
    });
    const failedAfterConfirmOpen = task({
      id: "failed-after-confirm-open",
      status: "error",
      error: "生成失败",
    });
    useStore.setState({ tasks: [failedAtConfirmOpen] });
    const failedTaskIds = useStore
      .getState()
      .tasks.filter((item) => item.status === "error")
      .map((item) => item.id);
    useStore.setState({ tasks: [failedAtConfirmOpen, failedAfterConfirmOpen] });

    await clearFailedTasks(failedTaskIds);

    expect(useStore.getState().tasks.map((item) => item.id)).toEqual([
      "failed-after-confirm-open",
    ]);
  });
});
...
describe("agent built-in image tool failure", () => {
  ...
  it("marks a started built-in image task as error when the stream fails", async () => {
    ...
    for (
      let i = 0;
      i < 10 && useStore.getState().tasks[0]?.status !== "error";
      i++
    ) {
      await new Promise((resolve) => setTimeout(resolve, 0));
    }
  ...
  it("marks a failed built-in image task as error while the Agent stream continues", async () => {
    vi.mocked(callAgentResponsesApi).mockImplementationOnce(async (opts) => {
      ...
      return {
        ...
        outputItems: [
          {
            type: "message",
            content: [{ type: "output_text", text: "图片失败，但回复继续。" }],
          },
        ],
    ...
    expect(
      state.agentConversations[0].messages.find(

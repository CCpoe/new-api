...
export default function SettingsModal() {
  ...
  const kkcodeIntegration = isKkcodeIntegration();
  const defaultConfigOnly = isDefaultConfigOnlyEnabled() || kkcodeIntegration;
  const activeProfile =
  ...
  const availableAgentProfiles = kkcodeIntegration
    ? draft.profiles.filter((profile) => profile.id === activeProfile.id)
    : draft.profiles;
  const agentTextProfiles = availableAgentProfiles.filter(
    isAgentTextApiProfile,
  );
  const selectedAgentTextProfile =
    ...
    (isAgentTextApiProfile(activeProfile)
      ? activeProfile
      : agentTextProfiles[0]) ??
    null;
  ...
  const updateAgentApiConfigMode = (mode: AgentApiConfigMode) => {
    commitSettings({
      ...
      agentTextProfileId:
        mode !== "off"
          ? (selectedAgentTextProfile?.id ?? draft.agentTextProfileId)
          : draft.agentTextProfileId,
      agentImageProfileId:
        mode === "hybrid"
          ? (selectedAgentImageProfile?.id ?? draft.agentImageProfileId)
                ...
                <div className="space-y-4">
                  ...
                  <label className="block">
                    ...
                    <input
                      ...
                      type="text"
                      disabled={kkcodeIntegration}
                      className={`w-full rounded-xl border border-gray-200/70 bg-white/60 px-3 py-2.5 text-sm text-gray-700 outline-none transition focus:border-blue-300 dark:border-white/[0.08] dark:bg-white/[0.03] dark:text-gray-200 dark:focus:border-blue-500/50 ${kkcodeIntegration ? "cursor-not-allowed opacity-70" : ""}`}
                    />
                  </label>

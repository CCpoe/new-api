...
function usesConcurrentOpenAIImageRequests(
  profile: ApiProfile,
  params: TaskParams,
) {
...
function createReadyAgentRecoveredToolState(
  round: AgentRound,
  tasks: TaskRecord[],
) {
...
function getAgentRecoveredToolCallCount(
  output: ResponsesOutputItem[],
  tasks: TaskRecord[],
) {
...
async function continueRecoveredAgentRound(taskId: string) {
  ...
  try {
    ...
    if (!recovered.allSuccessful) {
      failRound(
        getAgentRecoveredFailureError(updatedRound, updatedState.tasks),
      );
      return;
    }
...
async function executeAgentRound(
  ...
  try {
    ...
    const completeAgentImageTask = async (
      ...
      const actualParams: Partial<TaskParams> = {
        ...(Object.keys(image.actualParams ?? {}).length
          ? image.actualParams
          : {}),
        ...(!hasActualSizeParam(image.actualParams)
          ? (getImageSizeParam(stored) ?? {})
          : {}),
        n: 1,
      };
      updateTaskInStore(taskId, {
    ...
    if (resume) {
      ...
      if (batchImagesItem)
        apiInputForTurn.splice(apiInputForTurn.length - 1, 0, batchImagesItem);
    }
...
export async function importData(
  ...
  try {
    ...
    const data =
      selected.find((part) => part.manifest.settings)?.manifest ??
      selected[0].manifest;
